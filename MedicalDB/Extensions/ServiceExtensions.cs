...
namespace MedicalDB.Extensions
{
    ...
    {
        public static void ConfigureCors(this IServiceCollection services) => //когда обращаются с другого домейна (защита), запрос доступа
            services.AddCors(options =>
            {
                options.AddPolicy("CorsPolicy",
                builder => builder.AllowAnyOrigin().AllowAnyMethod().AllowAnyHeader()); //разрешены все домейны, все методы (POST, GET и тд), все хэдеры 
            });

        public static void ConfigureLoggerService(this IServiceCollection services) =>
            services.AddScoped<ILoggerManager, LoggerManager>(); //для каждого запроса новый объект (other: singleton, transient)
